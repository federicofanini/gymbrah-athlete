...
import { addTesterAction, getTesterCounts } from "@/actions/tester";
...
export function Tester() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await addTesterAction({ email, role: "athlete" });
  ...
  const isDisabled = !counts || counts.athlete.spotsLeft <= 0;
          ...
          <div className="w-full">
            <div className="text-center p-4 border rounded-lg bg-primary/5">
              <p className="text-sm text-muted-foreground mb-2">
                Available Athlete Spots
              </p>
              <p className="text-2xl font-bold text-primary">
                {counts?.athlete.spotsLeft} / {counts?.athlete.maximum}
              </p>
            </div>
          </div>
        )}
